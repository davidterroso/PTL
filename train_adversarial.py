...
pre_weight_file = './outputs/weights.h5'
...
def train_model():
    ...
    model_D.compile(optimizer=Adam(learning_rate=ADAM_LR, beta_1=ADAM_BETA_1), loss='categorical_crossentropy')

    model_G = retouch_unet(input_shape=(PATCH_SIZE_H, PATCH_SIZE_W, 3))
    model_G.compile(optimizer=Adam(learning_rate=ADAM_LR, beta_1=ADAM_BETA_1), loss=multiclass_balanced_cross_entropy_loss_unet)
    ...
    model_D_G.compile(optimizer=Adam(learning_rate=ADAM_LR, beta_1=ADAM_BETA_1), loss=['categorical_crossentropy', multiclass_balanced_cross_entropy_loss_unet])
    ...
    for e in range(0, EPOCH):
        ...
        model_G.save_weights('./outputs/final_gan_weights.h5')
